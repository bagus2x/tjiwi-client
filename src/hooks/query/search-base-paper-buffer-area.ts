...
const searchBasePaper = async ({ pageParam = defaultPageParam, queryKey }: QueryCtx) => {
  ...
  const url = `/basepapers/storage/${queryKey[1]?.storage_id}/search-in-buffer-area`;
...
export const QUERY_KEY = 'SEARCH_BASE_PAPERS_BUFFER_AREA';
...
const useSearchBasePaperBufferArea = () => {
...
export default useSearchBasePaperBufferArea;
